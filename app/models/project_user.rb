class ProjectUser < ApplicationRecord
    ...
    if paramsdata['aboutyou_image'].present?
      ...
      Dir.mkdir(Rails.root.join('app/assets/images/project/' + $project_id + '/project_user')) #Making Sub-Directory
      File.open(Rails.root.join('app/assets/images/project/' + $project_id + '/project_user/' + $original_filename), 'wb') do |file|
  ...
  def self.projectbeingeditedaboutyoudata(paramsdata)
    ...
    @connection = ActiveRecord::Base.connection
    result      = @connection.exec_query("SELECT * FROM project_users WHERE project_id = '#$projectid'")
    return result.to_a
