class ProjectAccount < ApplicationRecord
    ...
    def self.projectbeingeditedaccountdata(paramsdata)
        ...
        @connection = ActiveRecord::Base.connection
        result      = @connection.exec_query("SELECT * FROM project_accounts WHERE project_id = '#$projectid'")
        return result.to_a
    ...
    def self.saveeditaccount(paramsdata)
      ...
      @connection = ActiveRecord::Base.connection 
      result      = @connection.exec_query("UPDATE project_accounts SET fullname='#$edit_fullname',  	payment_type='#$edit_payment_type', bankemail='#$edit_bankemail', bankname='#$edit_bankname', account_number='#$edit_account_number', bank_ifsc_code='#$edit_bank_ifsc_code',  	account_type='#$edit_account_type', status='#$edit_status', updated_at='#$updated_at' WHERE project_id='#$projectid' AND id='#$account_id'")
