...
def self.saveRewards(paramsdata,projectid)
    ...
    result      = @connection.exec_query("INSERT INTO project_rewards (project_id, reward_tile, reward_amount, reward_description, reward_shipping_details, reward_estimated_delivery, status, created_at, updated_at) VALUES (#{ $value.map{ |i| '"%s"' % i }.join(', ') })")
    ...
    def self.projectbeingeditedrewarddata(paramsdata)
      ...
      @connection = ActiveRecord::Base.connection
      result      = @connection.exec_query("SELECT * FROM project_rewards WHERE project_id = '#$projectid'")
      return result.to_a
    ...
    def self.saveeditRewards(paramsdata)
     ...
     $total_values            = $edit_rewardtitle.count
      ...
      $updated_at
          ...
          result = @connection.exec_query("UPDATE project_rewards SET reward_tile='#{$edit_rewardtitle[i]}', reward_amount='#{$edit_pledgeAmounts[i]}',reward_description='#{$edit_reward_descriptions[i]}', 	reward_shipping_details='#{$edit_reward_shipping_details[i]}',reward_estimated_delivery='#{$edit_estimated_deliv_month[i]}',updated_at='#$updated_at' WHERE id='#{$rewardid[i]}'")
