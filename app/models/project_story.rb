...
#    end
    ...
    resultsavestory      = @connection.exec_query("INSERT INTO project_stories (project_id, project_description, project_risk, status, created_at, updated_at) VALUES ('#$project_id', '#$project_desc', '#$project_risk', '#$story_status', '#$created_at', '#$updated_at')")
  ...
  def self.projectbeingeditedstorydata(paramsdata)
    ...
    @connection = ActiveRecord::Base.connection
    result      = @connection.exec_query("SELECT * FROM project_stories WHERE project_id = '#$projectid'")
    return result.to_a
  ...
  def self.savingeditstory(paramsdata)
    ...
    if File.exist?(Rails.root.join('app/assets/images/project', 'projectvideos', $previous_uploaded_video))
      ...
      @connection = ActiveRecord::Base.connection 
      result      = @connection.exec_query("UPDATE project_stories SET project_video='#$originalvideo_filename', project_description='#$edited_project_desc', project_risk='#$edited_project_risk',  	status='#$edited_status', updated_at='#$updated_at' WHERE project_id='#$projectid' AND id='#$story_id'")
